...
export default function ForwardMessageModal({
  ...
  const forwardMessage = useForwardMessage(sourceConversationId);
  ...
  const busy = forwardMessage.isPending || startConversation.isPending;
  const recentChats = conversations.filter(
    (c) => c._id !== sourceConversationId
  );
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-2 pb-4">
          {debounced.length >= 1 ? (
            ...
            <p className="px-3 py-6 text-center text-sm text-muted-foreground">
              No other conversations — search for someone above
            </p>
          ) : (
            <>
